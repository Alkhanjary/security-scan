...
- `web_scan.py` — web scanner, runnable standalone: `py web_scan.py https://example.com --ai`. Beyond headers/cookies/TLS/XSS/SQLi/exposed-files/CORS/redirects: checks the actual *content* of an existing CSP for `unsafe-inline`/`unsafe-eval`/wildcard sources, flags cross-origin `<script>` tags missing Subresource Integrity, probes a guaranteed-404 path for a live debug/error page (Werkzeug, Django, Rails, PHP, ASP.NET, Node signatures), flags a versioned CMS/framework `<meta name="generator">` tag, checks the negotiated TLS cipher for RC4/3DES/NULL/export/anonymous suites, looks for a publicly-exposed JavaScript source map, sends a real TRACE request to confirm cross-site tracing (not just reading it off the `Allow` header), and checks any S3/GCS/Azure Blob bucket URL found on the page for a genuinely public listing.
- `network_scan.py` — network scanner, runnable standalone: `py network_scan.py 10.0.0.0/24 --ai`. Flags open ports, risky exposed services (FTP/Telnet/RDP/VNC, plus SMB/RPC/MSSQL/MySQL/PostgreSQL/Redis/Elasticsearch/MongoDB/Docker API/Memcached), and outdated service banners — matched offline against known-backdoored distributions (e.g. vsftpd 2.3.4) and per-product minimum-supported-version thresholds. No live CVE lookup: a hit means "go check CVEs for this exact version," not a confirmed exploit. Also runs a handful of active, single-request unauthenticated-access probes — FTP anonymous login, Redis `PING` with no auth, Elasticsearch cluster info with no auth, Docker Engine API version with no auth, VNC's RFB handshake checked for the "None" security type — each a read-only request/response, never brute-forcing or state-changing. SSH banners are checked for the obsolete protocol-1 identification string.
